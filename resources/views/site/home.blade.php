							...
							<div class="row">
								...
								@if (count($currentTop3) == 3)
									@foreach($currentTop3 as $controller)
										<?php $pos++ ?>
										@if ($pos == 1)
											<div class="col-md-6 col-xl-4"><a class="block block-link-pop bg-gd-sun text-center"><div class="block-content block-content-full"><img class="img-avatar" src="/storage/files/<?php echo $controller->cid; ?>.jpg" alt="Profile Picture"></div><div class="block-content block-content-full bg-black-op-5"><div class="font-w600 text-white mb-5">{{{ $controller->cid . " " . $controller->lname }}}</div><div class="font-size-sm text-white-op">{{{ $controller->duration_time }}}</div></div><div class="block-content block-content-full block-content-sm"><span class="font-w600 font-size-sm text-warning-light"><i class="fa fa-hourglass mr-10"></i>First Place</span></div></a></div>
										@elseif ($pos == 2)
