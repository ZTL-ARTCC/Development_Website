                ...
                @foreach($events as $e)
                    <tr>
                        @if($e->banner_path != null)
                            <td width="500px"><a href="/event/{{{$e->id}}}"><img src="{{ $e->banner_path }}"
                                                                                 width="500px" alt="{{ $e->name }}"></a>
                            </td>
                        @else
                        ...
                        @if(Auth::user()->can('events'))
                            <td>
                                @if($e->status == 0)
                                    <a href="/admin/events/set-active/{{ $e->id }}" class="btn btn-success"
                                       data-toggle="tooltip" title="Unhide Event"><i class="fas fa-check"></i></a>
                                @elseif($e->status == 1)
                                    <a href="/admin/events/hide/{{ $e->id }}" class="btn btn-warning"
                                       data-toggle="tooltip" title="Hide Event"><i class="fas fa-ban"></i></a>
                                @endif
                                <a href="/admin/events/edit/{{ $e->id }}" class="btn btn-success simple-tooltip"
                                   data-toggle="tooltip" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                <a href="/admin/events/delete/{{ $e->id }}" class="btn btn-danger simple-tooltip"
                                   data-toggle="tooltip" title="Delete"><i class="fas fa-times"></i></a>
                                @if($e->status == 0)
                                    <br>
