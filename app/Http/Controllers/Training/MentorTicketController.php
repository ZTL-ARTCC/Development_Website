...
class MentorTicketController extends Controller {
    ...
    public function addTicket(Request $request) {
        ...
        $c = $request->id;
        $user = User::where('status', '1')->where('canTrain', '1')->orderBy('lname', 'ASC')->get()
    ...
    public function saveTicket(Request $request) {
        ...
        $audit->what = Auth::user()->full_name . ' added a training ticket for ' .
                       User::find($ticket->controller_id)->full_name . '.';
        $audit->save();


        return redirect('/dashboard/training/tickets?id=' . $ticket->controller_id)->with('success',
