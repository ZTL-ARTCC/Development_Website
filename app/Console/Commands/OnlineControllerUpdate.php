...
class OnlineControllerUpdate extends Command {
    ...
    protected $facilities = [
        ...
        /* DELTA */
        'AGS', 'GMU', 'GYH', 'TCL', 'MXF', 'MGM', 'LSF', 'CSG', 'MCN', 'WRB', 'JQF', 'VUJ', 'INT', 'TRI', 'LZU',
        'ASN', 'HKY', 'PDK',
        /* OBSERVER */
        'ZTL'
    ];
    ...
    public function handle() {
        ...
        foreach ($statsData as $line) {
            ...
            if ($client_section) {
                list($position, $cid, $name, $clienttype, $frequency, $latitude, $longitude, $altitude,
                    $groundspeed, $planned_aircraft, $planned_tascruise, $planned_depairport, $planned_altitude,
                    $planned_destairport, $server, $protrevision, $rating, $transponder, $facilitytype,
                    $visualrange, $planned_revision, $planned_flighttype, $planned_deptime, $planned_actdeptime,
                    ...
                    $time_last_atis_received, $time_logon, $heading) = explode(":", $line);
                ...
                if ($is_controller) {
                    // Found an ATC user
                    $time_logon = strtotime(preg_replace("/^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})$/",
                                                         "$1-$2-$3 $4:$5:$6", $time_logon));
