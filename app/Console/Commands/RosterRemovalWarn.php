...
class RosterRemovalWarn extends Command {
    ...
    public function handle() {
        ...
        $student =
            User::where('rating_id', 2)->orWhere('rating_id', 3)->orWhere('rating_id', 4)->where('canTrain', 1)
                ->where('status', 1)->where('visitor', 0)->get();
        $controller =
            User::where('rating_id', 5)->orWhere('rating_id', 7)->orWhere('rating_id', 8)->orWhere('rating_id', 9)
                ->orWhere('rating_id', 10)->orWhere('rating_id', 11)->orWhere('rating_id', 12)->where('canTrain', 1)
                ->where('status', 1)->where('visitor', 0)->get();
        ...
        foreach ($obs as $s) {
            ...
            if ($last_training == null || strtotime($t->date . ' ' . $t->start_time) < strtotime($last_month)) {
                Mail::send('emails.inactive.obs', ['s' => $s], function($message) use ($s) {
                    $message->from('activity@notams.ztlartcc.org', 'vZTL ARTCC Activity Department')
